...
function animateCartIndicator() {
    const target =
        document.querySelector(".open-cart-count") ||
        document.querySelector(".open-cart");
    if (!target) return;

    const rect = target.getBoundingClientRect();
    const badgePlus = document.createElement("span");
    badgePlus.classList.add("plus-one", "cart-plus-one");
    badgePlus.textContent = "+1";
    badgePlus.style.left = rect.left + rect.width / 2 + "px";
    badgePlus.style.top = rect.top - 8 + "px";
    document.body.appendChild(badgePlus);
    setTimeout(() => badgePlus.remove(), 900);
}
...
function renderCartFloat() {
    ...
    const countEls = document.querySelectorAll("#cartCount, .open-cart-count");
    ...
    if (!cart || cart.length === 0) {
        ...
        if (itemsCountEl) itemsCountEl.textContent = "Nenhum item ainda";
        return;
    }
    ...
    if (itemsCountEl) {
        itemsCountEl.textContent = count === 1 ? "1 item selecionado" : `${count} itens selecionados`;
    }
}
...
function toggleCart(forceState) {
    ...
    if (!el) return;
    const shouldOpen = typeof forceState === "boolean" ? forceState : !el.classList.contains("show");
    el.classList.toggle("show", shouldOpen);
    el.setAttribute("aria-hidden", shouldOpen ? "false" : "true");
    document.body.classList.toggle("cart-visible", shouldOpen);
...
function completeOrderFlow(feedback) {
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    window.completeOrderFlow = completeOrderFlow;
